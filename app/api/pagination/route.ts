...
import { getProjectsPagination } from "@/app/services/project.services";
...
export async function GET(request: Request) {
  ...
  try {
    const result = await getProjectsPagination({ limit, offset });
    ...
    return NextResponse.json(
      { message: getApiErrorMessage(error, "Failed to fetch projects") },
