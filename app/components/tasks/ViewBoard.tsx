...
export default function ViewBoard({
  ...
  const tasksByStatus = useAppSelector(
    (state) => state.tasks.tasksByStatus,
  );

  const statusByColumn = useAppSelector(
    (state) => state.tasks.statusByColumn,
  );

  const boardProjectId = useAppSelector(
    (state) => state.tasks.boardProjectId,
  );
  ...
  if (showEmptyState) {
    return (
      <div className="mt-8 flex min-h-[40vh] w-full flex-col items-center justify-center gap-2">
        ...
        <p className="text-2xl font-semibold text-[#041B3C]">
          No tasks found
        </p>
